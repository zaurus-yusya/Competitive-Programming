...
int main() {
    ...
    //登るが1通り、降りるが2通り
    //登った後は降りれない　ただし一番上まで登った時のみ1通りで降りれ、その後は降りるだけ

    //深さ 0-indexed
    ll depth = n-1;
    //cerr << "depth = " << depth << endl;
    //Dの時前計算 x回登った時の
    vector<mint> toori(d+1);
    bool f = false;
    ll cnt = d;
    rep(i, d+1){
        if(f){
    ...
    //累積和
    vector<mint> toorisum(d+2);
    rep(i, d+1){
        toorisum[i+1] = toorisum[i] + toori[i];
    }

    //vecdbg(toorisum);

    vector<ll> kudari(d+1);
    ll tmp = -d;
    rep(i, d+1){
        kudari[i] = tmp;
        tmp += 2;
    ...
    for(ll i = 0; i <= depth; i++){
        //nodeの数は2^i個
        mint nodenum = mpow.modpow(2, i);

        //登れる回数
        ll up = min(i, d);
        //降りれる回数
        ll down = i - depth;
        ll dis = lower_bound(kudari.begin(), kudari.end(), down) - kudari.begin();
        

        //cout << "i = " << i << " dis = " << dis << endl;
        //cout << nodenum << " " << toorisum[up+1] << " " << toorisum[dis] << endl;
        
        if(dis >= up+1) continue;
        ans += nodenum * (toorisum[up+1] - toorisum[dis]);


    }
    //cout << toorisum.size() << endl;
    cout << ans << endl;
