...
int main() {
    ...
    mint ans = 0;
    for(ll i = abs(z); i <= n; i += 2){
        //zの回数
        ll znum = i;
        ll zc = (i - abs(z)) / 2;
        mint zres = com.ncr(znum, zc);
        ...
        if(abs(X) > num || abs(Y) > num) continue;
        if( (num % 2) != (abs(X) % 2) || (num % 2) != (abs(Y) % 2)) continue;

        mint xyres = com.ncr(num, xc) * com.ncr(num, yc);
        
        ans += zres * xyres * com.ncr(n, znum);
    }
    cout << ans << endl;
