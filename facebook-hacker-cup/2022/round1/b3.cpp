...
int main() {
    ...
    rep(T, t){
        ...
        ll x_sum = 0;
        ll y_sum = 0;
        vector<ll> tree_x_sum(n+1);
        vector<ll> tree_y_sum(n+1);
        rep(i, n){
            tree_x_sum[i+1] = tree_x_sum[i] + tree_x[i];
            tree_x_sum[i+1] %= mod;
            tree_y_sum[i+1] = tree_y_sum[i] + tree_y[i];
            tree_y_sum[i+1] %= mod;
        ...
        rep(i, n){
            ans += (mint)tree_x[i] * (mint)tree_x[i] * q;
            ans += (mint)tree_y[i] * (mint)tree_y[i] * q;

            // ans += ((tree_x[i] % mod) * (tree_x[i] % mod) % mod) * q;
            // ans += ((tree_y[i] % mod) * (tree_y[i] % mod) % mod) * q;
        }
        ...
        rep(i, q){
            ...
            ans -= (mint)2 * x * tree_x_sum[n];
            ans -= (mint)2 * y * tree_y_sum[n];
            ...
            ans += ((x * x) % mod) * n;
            // ans -= (2 * x * tree_x_sum[n]);
            ans -= (((2 * x) % mod) * x_sum);

            ans += ((y * y) % mod) * n;
            // ans -= (2 * y * tree_y_sum[n]);
            ans -= (((2 * y) % mod) * y_sum);
            */

        }
        // ans -= 2 * (ll)tree_x_sum[n] * (ll)ido_x_sum[n];
        // ans -= 2 * (ll)tree_y_sum[n] * (ll)ido_y_sum[n];
