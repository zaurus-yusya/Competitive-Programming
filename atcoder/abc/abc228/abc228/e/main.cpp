...
template<long long mod2> struct MPow2 {
    using mint2 = ModInt<mod2>;
    mint2 modpow2(mint2 a, long long n) {
        mint2 res = 1;
            while(n > 0){
            if(n & 1){
                res = res * a;
...
typedef ModInt<998244352> mint2;
MPow<998244353> mpow;
MPow2<998244352> mpow2;

int main() {
    std::cout << std::fixed << std::setprecision(15);
    ll n, k, m; cin >> n >> k >> m;

    mint2 x = mpow2.modpow2(k, n);
    if(x == 0 && (m % 998244353) == 0){
        cout << 0 << endl; return 0;
    }

    mint ans = mpow.modpow(m, (ll)x);
    cout << ans << endl;
}