...
//建てる場所に優先順位をつけてみた
int tugi = 0;
vector<pair<int, int>> junban_b = {
    {7, 7}, {7, 8}, {7, 6}, {7, 9}, {7, 5}, {7, 10}, {7, 4}, {7, 11}, {7, 3}, {7, 12}, {7, 2}, {7, 13}, {7, 1}, {7, 14}, {7, 0}, {7, 15},
    {8, 8}, {6, 8}, {9, 8}, {5, 8}, {10, 8}, {4, 8}, {11, 8}, {3, 8}, {12, 8}, {2, 8}, {13, 8}, {1, 8}, {14, 8}, {0, 8}, {15, 8}
};
vector<pair<int, int>> junban;
vector<int> junbanm = {7, 8, 6, 9, 5, 10, 4, 11, 3, 12, 2, 13, 1, 14, 0, 15};
...
struct Game {
    ...
    void simulate(int day, const Action& action) {
        ...
        /* appearはアクションを決める際に次の日の状態をあらかじめ見ておいて、決めることにする
        // appear
        for (const Vegetable& vege : veges_start[day]) {
            vege_values[vege.r][vege.c] = vege.v;
        }
    ...
    //action5以降 どの行と列で十字架作るか計算
    void calc_cross_rc(){
        vector<vector<int>> sum_future_veges(N, vector<int>(N, 0));
        //X_DAY以降に出てくるコストの合計をマスごとに計算
        for (int i = X_DAY; i < T; i++) {
            for (const Vegetable& vege : veges_end[i]) { //終了日がX_DAY以降に出てくるもの(X_DAY以降に存在するもの)
    ...
    Node calc_most_earn_place(){
        ...
        for(int r = 0; r < N; r++){
            for(int c = 0; c < N; c++){
                ...
                if(has_machine[r][c]) continue; //todo 移動するマシンの場合、コストを計算した方が良い
