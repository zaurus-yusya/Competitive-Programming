#include <bits/stdc++.h>
#include <atcoder/all>
typedef long long ll;
typedef long double ld;
#define rep(i,n) for(ll i=0;i<(n);i++)
#define repr(i,n) for(ll i=(n-1);i>=0;i--)
#define all(x) x.begin(),x.end()
#define br cout << "\n";
using namespace std;
using namespace atcoder;
const long long INF = 1e10;
const long long MOD = 1e9+7;
using Graph = vector<vector<ll>>;
using pll = pair<ll, ll>;
template<class T> inline bool chmin(T &a, T b) { if(a > b){ a = b; return true;} return false;}
template<class T> inline bool chmax(T &a, T b) { if(a < b){ a = b; return true;} return false;}
ll ceilll(ll a, ll b) {return (a + b-1) / b;} // if(a%b == 0) (a/b) + 1
ll get_digit(ll a) {ll digit = 0; while(a != 0){a /= 10; digit++;} return digit;} // a != 0
template<typename T> void vecdbg(vector<T>& v){ rep(i, v.size()){cout << v[i] << " ";} br;}
template<typename T> void vecvecdbg(vector<vector<T>>& v){ rep(i, v.size()){rep(j, v[i].size()){cout << v[i][j] << " ";} br;}}

// 0 false, 1 true 
// string number to int : -48
// a to A : -32
// ceil(a)  1.2->2.0
// c++17	g++ -std=c++17 a.cpp
// global vector -> 0 initialization

int main() {
    std::cout << std::fixed << std::setprecision(15);
    ll n; cin >> n;
    vector<long long> A(n+1);
    vector<long long> B(n+1);
    vector<long long> C(n+1, -1);
    vector<pair<ll, ll>> P(n+1);

    
    vector<pair<ll, ll>> OK;
    vector<pair<ll, ll>> Left;
    vector<pair<ll, ll>> Right;
    vector<pair<ll, ll>> None;


    map<ll, ll> mp;
    rep(i, 2*n){mp[i] = -1;}
    for(long long i = 0; i < n; i ++){
        cin >> A[i] >> B[i];
        if(A[i] != -1 && B[i] != -1){
            mp[A[i]] = i; mp[B[i]] = i;
            OK.emplace_back(A[i], B[i]);
        }else if(A[i] != -1 && B[i] == -1){
            mp[A[i]] = i;
            Left.emplace_back(A[i], B[i]);
        }else if(A[i] == -1 && B[i] != -1){
            mp[B[i]] = i;
            Right.emplace_back(A[i], B[i]);
        }else{
            None.emplace_back(A[i], B[i]);
        }
    }

    sort(all(OK));

    cout << "OK" << endl;
    rep(i, OK.size()){
        cout << OK[i].first << " " << OK[i].second << endl;
    }
    cout << "Left" << endl;
    rep(i, Left.size()){
        cout << Left[i].first << " " << Left[i].second << endl;
    }
    cout << "Right" << endl;
    rep(i, Right.size()){
        cout << Right[i].first << " " << Right[i].second << endl;
    }
    cout << "None" << endl;
    rep(i, None.size()){
        cout << None[i].first << " " << None[i].second << endl;
    }

}