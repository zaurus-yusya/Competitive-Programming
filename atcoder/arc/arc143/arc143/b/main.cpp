...
int main() {
    ...
    for(ll i = 1; i <= n*n; i++){
        ...
        // 行 n-1マスにiより大きい数を置く
        mint tmp_ma = com.npr(n*n - i, n-1);

        // 列 n-1マスにiより小さい数を置く
        mint tmp_mi = com.npr(i - 1, n-1);

        // 残りのマスの置き方
        mint tmp_nokori = kaijo[n*n - 2 * n + 1];

        yozisyo += tmp * tmp_ma * tmp_mi * tmp_nokori;

    }

    mint all_ = kaijo[n*n];

    mint ans = kaijo[n*n] - yozisyo;
