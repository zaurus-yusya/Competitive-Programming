...
int main() {
    ...
    ll n, m, k; cin >> n >> m >> k;
    vector<vector<mint>> dp(1010, vector<mint>(1010));
    // i回振ってjのマスにいる確率
    dp[0][0] = 1;

    for(ll i = 0; i < k; i++){ // i回目

        // マスnに達した場合は終了(それ以上サイコロ振らない)
        for(ll j = 0; j < n; j++){ // jのマスからサイコロ振る
            for(ll k = 1; k <= m; k++){ // kの目を出す
                ...
                dp[i+1][next] += dp[i][j] * (mint)1/(mint)m;
            }
        }
    }
