...
mint calc2(ll tate, ll yoko){
    mint res = 0;

    mint odd_yoko_kosu = (yoko + 1) / 2;
    mint odd_yoko_last = yoko; if(yoko % 2 == 0) odd_yoko_last -= 1;
    mint odd_yoko_sum = (odd_yoko_last + 1) * odd_yoko_kosu / 2;
    
    mint odd_tate_first = odd_yoko_sum;
    mint odd_tate_kosu = (tate + 1) / 2;
    mint odd_tate_last = odd_tate_first + (odd_tate_kosu - 1) * m*2 * odd_yoko_kosu;
    mint odd_res = (odd_tate_first + odd_tate_last) * odd_tate_kosu / 2;
    

    mint even_yoko_kosu = yoko / 2;
    mint even_yoko_last = yoko+m; if(yoko % 2 == 1) even_yoko_last -= 1;
    mint even_yoko_sum = (even_yoko_last + 2+m) * even_yoko_kosu / 2;

    mint even_tate_first = even_yoko_sum;
    mint even_tate_kosu = tate / 2;
    mint even_tate_last = even_tate_first + (even_tate_kosu - 1) * m*2 * even_yoko_kosu;
    mint even_res = (even_tate_first + even_tate_last) * even_tate_kosu / 2;

    return odd_res + even_res;
}
...
int main() {
    ...
    rep(i, q){
        ...
        a--; c--;
        mint ans = calc2(b, d) - calc2(b, c) - calc2(a, d) + calc2(a, c);
        cout << ans << endl;
    }
