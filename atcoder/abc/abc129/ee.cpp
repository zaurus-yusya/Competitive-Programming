    ...
    for(ll i = 0; i < s.size(); i++){
        ll num = s[i] - '0';
            ...
            if(num == 0){
                ...
                dp[i+1][1][0] += dp[i][1][0] * 3;
                dp[i+1][1][0] += dp[i][1][1] * 3;
            }else{
                dp[i+1][0][1] += dp[i][0][0] * 2;
                dp[i+1][0][1] += dp[i][0][1] * 2;

                dp[i+1][1][1] += dp[i][0][0] * 1;
                dp[i+1][1][1] += dp[i][0][1] * 1;

                dp[i+1][1][1] += dp[i][1][0] * 3;
                dp[i+1][1][1] += dp[i][1][1] * 3;
            }
            
    }

    cout << dp[s.size()][0][0] +  dp[s.size()][0][1] + dp[s.size()][1][0] + dp[s.size()][1][1] << endl;
