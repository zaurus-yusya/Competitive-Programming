...
int main() {
    ...
    mint ans = kaijo[w+b] / (kaijo[w]*kaijo[b]);
    cout << ans << endl;
    ...
    rep(i, w+b){
        ...
        for(ll j = wnum + 1; j <= r; j++){
            ll ww = w - j;
            ll bb = (i+1) - ww;
            mint yy = kaijo[j + (i+1) - j] / (kaijo[j]*kaijo[(i+1)-j]);
            mint xx = kaijo[ww+bb] / (kaijo[ww]*kaijo[bb]);
            
            //cout << i+1 << " " << xx << " " << yy << endl;
            ans -= yy*xx;
        }

        //cout << i+1 << " : " << l << " " << wnum << " " << r << endl;
    }
