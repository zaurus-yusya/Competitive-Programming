...
int main() {
    ...
    // 初期(0, 0)
    // n回ワープ 300
    // 到達する点の集合, valueは移動経路の総数
    map<pair<ll, ll>, mint> current;
    current[{0, 0}] = 1;
    
    for(ll i = 0; i < n; i++){
        // currentの状態から1回ワープした時の到達する点の集合
        map<pair<ll, ll>, mint> next;

        // nextを求める
        for(auto&j: current){
            ll now_x = j.first.first;
            ll now_y = j.first.second;
            mint p = j.second;

            //3パターンのワープ
            for(ll k = 0; k < 6; k+=2){
                ll next_x = now_x + warp[k];
                ll next_y = now_y + warp[k+1];

                // next_y, next_xが行けないマスだったらcontinue
                if(cant.count({next_x, next_y})){
                    continue;
                }
                next[{next_x, next_y}] += p;
            }
        }
