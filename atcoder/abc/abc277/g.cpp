...
int main() {
    ...
    map<ll, map<ll, mint>> joutai; // node, level, kakuritu
    joutai[0][0] = 1;

    vector<mint> kitai(n);

    for(ll i = 0; i < k; i++){ // 3000
        map<ll, map<ll, mint>> res; // node, level, kakuritu

        for(auto &j: joutai){
            ll node = j.first;
            mint senni = (mint)1 / (mint)g[node].size();
            
            for(auto &tmp: j.second){
                ll level = tmp.first;
                mint kaku = tmp.second;

                for(ll x = 0; x < g[node].size(); x++){
                    ll next = g[node][x];

                    if(c[next] == 0){
                        // レベルアップ
                        res[next][level+1] += kaku * senni;
                    }else{
                        // 期待値
                        res[next][level] += kaku * senni;
                        kitai[next] += kaku * senni * level * level;
                    }
