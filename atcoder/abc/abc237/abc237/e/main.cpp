...
int main() {
    ...
    rep(i, m){
        ll u, v; cin >> u >> v; u--; v--;
        if(h[u] >= h[v]){
            g2[u].emplace_back(v, (h[u] - h[v]) -1 * (h[u] - h[v]));
            g2[v].emplace_back(u, (h[u] - h[v]));
        }else{
            g2[u].emplace_back(v, (h[v] - h[u]));
            g2[v].emplace_back(u, (h[v] - h[u]) -1 * (h[v] - h[u]));
        }
    }
