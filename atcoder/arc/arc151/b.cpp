...
int main() {
    ...
    vector<vector<mint>> dp(200010, vector<mint>(2, 0));
    dp[0][0] = 1;
    dp[0][1] = 1;
    // 0: 同じ, 1: 未満
    ...
    for(ll i = 0; i < n; i++){
        if(p[i] == i){
            ...
            if(mp.count(i) > 0){
                ...
                // 同じ -> 未満
                dp[i+1][1] += dp[i][0] * com.ncr(m, 2);

                // 未満 -> 未満
                dp[i+1][1] += dp[i][1] * mpow.modpow(m, 2);
    ...
    cout << dp[n][1] / mpow.modpow(2, cnt) << endl;
